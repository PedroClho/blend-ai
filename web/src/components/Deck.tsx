    ...
    >
      ...
      <div className="p-5">
        ...
        <button
          ...
          className={`mt-4 flex w-full cursor-pointer flex-col items-center justify-center gap-1.5 rounded-lg border border-dashed px-4 py-7 transition-colors ${
            arquivo
              ? "border-line bg-surface"
              : "border-ink-faint/40 bg-surface/60 hover:border-brand hover:bg-brand-soft/40"
          }`}
        >
          {arquivo ? (
            <>
              <span className="max-w-full truncate font-mono text-[12px] font-medium text-ink">
                {arquivo.name}
              </span>
              <span className="microlabel">
                {(arquivo.size / 1024 / 1024).toFixed(1)} MB — trocar
              </span>
            </>
          ) : (
            <>
