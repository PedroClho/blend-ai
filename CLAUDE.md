...
- **Ancoragem manual na app (2026-07-03):** UI ganhou a estratégia **"Manual (DJ)"** — análise prévia por faixa (`POST /api/analyses`, cache por hash de conteúdo em `data/cache/analises/`; o job de mashup reusa o cache e pula allin1/Essentia), clique na waveform ancora o vocal com snap em downbeat, stepper de compassos e toggle de transposição (H3). `POST /api/mashups` aceita `vocal_in`/`vocal_dur`/`vocal_offset`/`transpor`; âncoras marcam `plan.mode="manual"` e re-rotulam a seção alvo. Spec: `specs/ancoragem-manual.md`.
